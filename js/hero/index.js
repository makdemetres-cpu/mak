...
import { Vector3 } from '../vendor/three.slim.js?v=260830a';
import { createRig, detectTier, hasWebGL, makeResizer } from './scene.js?v=260830a';
import { buildVilla, makeMaterials } from './villa.js?v=260830a';
import { skinVilla } from './skin.js?v=260830a';
import { CHAPTERS, chapterAt, clamp01, doorAngle, evaluate } from './path.js?v=260830a';

/* The hero is the modelled villa and the camera flight it always was — and
   every surface on it is now a patch cut from the photographs of the real
   house, rather than a flat colour picked by hand. See skin.js.

   Two other modules sit in this folder unimported: photos.js, which put the
   seven photographs on planes in front of the model, and walk.js, which
   walked a camera through them as a corridor. Both were tried in the hero and
   neither is what was wanted; they are kept because this hero has changed
   direction more than once. See README → "The hero". */
...
function boot() {
  ...
  /* The photographs, as the villa's surfaces. They arrive over the network
     well after the first frame, so each one wakes the loop — otherwise the
     hero sits there in flat colour until the visitor happens to scroll. */
  skinVilla(mats, () => wake());
