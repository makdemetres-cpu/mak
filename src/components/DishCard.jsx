...
export function DishCard({ name, weight, desc, price, tag, image }) {
  const { ref, style, handleMove, handleLeave } = useTilt();

  return (
    <motion.article
      ref={ref}
      className={`cut-card ${image ? 'cut-card--photo' : ''}`}
      onMouseMove={handleMove}
      onMouseLeave={handleLeave}
      style={style}
    >
      {image && (
        <div className="cut-card__photo" style={{ backgroundImage: `url(${image})` }} aria-hidden="true" />
      )}
